...
def get_prediction_mask(info_dict):
    ...
    # Create return value.
    return_value = torch.zeros((info_dict[constant.detectron.HEIGHT_KEY], info_dict[constant.detectron.WIDTH_KEY]), dtype=torch.int)
    ...
    for i, pred_box in enumerate(b_boxes):
        ...
        return_value[crop_y0: crop_y1, crop_x0: crop_x1][patch_prediction == 1] = instance_label
    ...
    return_value_array = return_value.cpu().numpy()
    return_value_array = return_value_array.astype("int32")
    # return_value_array = np.transpose(return_value_array, (1, 2, 0))    # We don't have the channel dimension.
    im = Image.fromarray(return_value_array)
    im.save(f"/tmp/predicted_masks/{base_filename}", mode="I")
...
def get_prediction_mask_from_cache(info_dict):
    ...
    image = Image.open(f"/tmp/predicted_masks/{base_filename}")
    arr = np.array(image)
    return_value = torch.from_numpy(arr)
    return_value = return_value.cuda()
    return return_value
...
def save_results_to_csv():
    ...
    for i in tqdm.tqdm(range(len(my_data_list)), position=0, leave=True):
        ...
        filename = sample[constant.detectron.FILENAME_KEY]
        base_filename = os.path.basename(filename)
        if (not os.path.exists(f"/tmp/predicted_masks/{base_filename}")):
            pred_mask = get_prediction_mask(sample)    # NOTE: Calculate once
        else:
